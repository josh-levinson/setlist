...
// Main seeding function with authentication
export const seedDatabaseWithAuth = async (userId: string, userEmail: string, userPassword: string, options: {
  jokeCount?: number
  setlistCount?: number
  tagCount?: number
} = {}) => {
...
if (require.main === module) {
  ...
  if (!userId || !userEmail || !userPassword) {
    console.error('❌ Please provide user ID, email, and password as arguments')
    console.log('Usage: bun run scripts/seed-with-auth.ts <user-id> <email> <password>')
    process.exit(1)
  }
  
  const jokeCount = parseInt(process.argv[5]) || 50
  const setlistCount = parseInt(process.argv[6]) || 10
  const tagCount = parseInt(process.argv[7]) || 15
  
  seedDatabaseWithAuth(userId, userEmail, userPassword, { jokeCount, setlistCount, tagCount })
    .then((result) => {
